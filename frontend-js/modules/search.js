...
export default class Search {
  // 1. Select DOM elements, and keep track fo any useful data
  constructor() {
    // storing matching csrf token value(for CSRF security)
    ...
    // property to keep track of typing
  ...
  renderResultHTML(posts) {
    if (posts.length) {
      ...
      this.resultsArea.innerHTML = `<p class='alert alert-danger text-center shadow-sm'> Sorry, couldn't find any match for that search</p>`
    }
  ...
  openOverlay() {
    ...
    // the div that this following field lives within was hidden until the above line of code ran, some browsers will run into issues, that's why we are delaying using setTimeout()
